#!/home/jack/Desktop/content/env/python
import subprocess
import logging
import imageio
...
def create_avatar():
    if request.method == 'POST':
        ...
        build_avatar = f"/home/jack/Desktop/content/env/bin/python3.8 inference.py --checkpoint_path {check_point} --face {video_path} --audio {wav_filename}"
...
def mkblend_video():
    ...
    #DIR = "/home/jack/Desktop/content_copy/static/civitai/"
    DIR = "/home/jack/Desktop/content_copy/static/dreamlike_art/"
    # Get a list of image files
    image_list = glob.glob(DIR + "*.jpg")
    
    # Shuffle and select a subset of images
    random.shuffle(image_list)
    image_list = random.sample(image_list, 15)
    ...
    imagelist = sorted(glob.glob('static/animate_temp/*.jpg'), key=os.path.getmtime)
    ...
    File = 'static/animate_temp/'+filename
    logger.debug('This is File: %s', File)
    clip.write_videofile(File, fps=24, codec='libx265', preset='medium')
    shutil.copy(File, "static/animate_temp/TEMP3.mp4")
    video = "static/animate_temp/TEMP3.mp4"
    # Usage example
    input_file = "static/animate_temp/TEMP3.mp4"
    output_file = "repaired_video.mp4"

    repair_corrupted_mp4(input_file, output_file)
    webp_path = 'static/animate_temp/TEMP3a.webp'
    frame_rate = 24
    ...
    while not os.path.exists(video) and attempts < max_attempts:
        ...
        print(f'Waiting for {video} to be created...',attempts)
        attempts += 1

    if os.path.exists(video):
        return render_template('mkblend_video.html', video=webp_path)
    else:
